...
def test_03_mean_rating_by_genre(current_test_name):
    try:
        try:
            import pandas as pd
        except ImportError:
        ...
        pickle_file_path = os.path.join(CURRENT_DIR, '03.xlsx')
        # Expected data
        ...
        expected_file_name = r"mean_rating_by_genre.xlsx"
        ...
        try:
            pd.testing.assert_frame_equal(actual_data, expected_data, check_exact=False, check_dtype=False, atol=.05)
            rec.pass_case(None)
        except AssertionError as e:
            formatted = format_error_message(
            custom_message=(f"Your `{expected_file_name}` file didn't contain the expected data:\n"
                            f"### Expected excel file values:\n"
                            f"```\n{expected_data_str}\n```\n"
                            f"### Your excel file values:\n"
                            f"```\n{actual_data_str}\n```\n"
                            f"### How to fix it:\n"
                            f"Make sure you are choosing the correct column in .groupby and using the correct math function. "
                            f"Also make sure you are exporting the results of the .groupby function, not another dataframe."),
            current_test_name=current_test_name,
