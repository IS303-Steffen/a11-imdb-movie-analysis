...
def test_02_newer_comedies(current_test_name):
    try:
        try:
            import pandas as pd
        except ImportError:
        ...
        expected_file_name = r"newer_comedies.xlsx"
        ...
        actual_data = actual_data['title']
        actual_data_str = actual_data.to_string(index=False).strip()

        try:
            pd.testing.assert_series_equal(actual_data, expected_data, check_exact=False, check_dtype=False, atol=1)
            rec.pass_case(None)
        except AssertionError as e:
            formatted = format_error_message(
            custom_message=(f"Your `{expected_file_name}` file didn't contain the expected movie titles:\n"
                            f"### Expected excel file values:\n"
                            f"```\n{expected_data_str}\n```\n"
                            f"### Your excel file values:\n"
                            f"```\n{actual_data_str}\n```\n"
                            f"### How to fix it:\n"
                            f"You likely aren't filtering your data correctly in .query, or aren't saving the results of your filtering before exporting your dataframe to excel."),
            current_test_name=current_test_name,
